...
import type { WorkspaceChooserStrategy } from "@/lib/workspaceChooser";
...
const props = defineProps<{
  ...
  workspaceChooserStrategy?: WorkspaceChooserStrategy;
}>();
...
const hasWorkspace = computed(() =>
  Boolean(String(props.workspace || "").trim()),
);
const workspaceChooserUnavailable = computed(
  () =>
    props.workspaceChooserSupported === false &&
    props.workspaceChooserStrategy !== "browser",
);
const workspaceChooseTitle = computed(() => {
  if (workspaceChooserUnavailable.value) {
    return props.workspaceChooserHint || "当前环境不支持选择文件夹";
  }
  return props.workspaceChooserStrategy === "browser"
    ? "浏览服务端目录（服务端）"
    : "选择工作区文件夹";
});
</script>
    ...
    <div class="max-w-3xl w-full space-y-12 text-center">
      ...
      <div class="space-y-6 animate-fade-in-up">
        <div class="relative inline-block">
          ...
          ></div>
          <div
            class="relative bg-surface-900 ring-1 ring-surface-800 p-6 rounded-3xl shadow-2xl"
          >
        ...
        <div class="space-y-3">
          <h1
            class="text-4xl md:text-5xl font-bold tracking-tight text-[var(--color-text-main)]"
          >
      ...
      <div v-if="hasWorkspace || showWorkspacePrompt" class="space-y-3">
        <div
          class="mx-auto max-w-2xl rounded-2xl bg-surface-900/40 backdrop-blur border border-surface-800 px-5 py-4 text-left"
        >
          <div class="flex items-start justify-between gap-4">
            <div class="min-w-0">
              ...
              <p
                v-if="hasWorkspace"
                class="mt-2 text-xs text-surface-400 break-words"
              >
                {{ workspace }}
                <span v-if="workspaceSource" class="text-surface-500">
              ...
              <div
                v-if="
                  Array.isArray(runtimeWarnings) && runtimeWarnings.length > 0
                "
                class="mt-2 space-y-1"
              >
                <p
                  v-for="(w, idx) in runtimeWarnings"
                  :key="idx"

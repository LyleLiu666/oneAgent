...
#### Scenario: 网页目录浏览器隐藏 oneAgent 内部状态目录
- **GIVEN** 服务端开放了网页目录浏览器
- **AND** 某个允许浏览的根目录下包含 `ONEAGENT_HOME/.oneagent/` 等内部状态目录
- **WHEN** 用户在网页目录浏览器中浏览该根目录
- **THEN** 系统不得把这些内部状态目录展示为可进入、可选择的 workspace 候选

#### Scenario: 某些根目录只用于浏览，不可直接作为 workspace
- **GIVEN** 服务端开放了网页目录浏览器
- **AND** 某个允许浏览的根目录仅用于承载导航入口或内部状态
- **WHEN** 用户进入该目录
- **THEN** 系统仍允许继续浏览其子目录
- **BUT** 系统必须明确提示该目录不能直接被选为 workspace

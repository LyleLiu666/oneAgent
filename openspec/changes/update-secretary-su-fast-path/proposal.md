...
## Why
当前秘书（Secretary）在用户消息进入后，通常会走 SW 规划 + 派发 worker 的链路。对于“预计可在约 <=5 次只读工具调用内闭环（仅作决策阈值，非硬限制）”的小请求（例如查询/解释/核对事实），这种链路会带来不必要的：
...
## What Changes
- SU 负责 triage 规划与短路径自解：先判断是否大概率在约 `<=5` 步只读工具调用内闭环（决策阈值，非硬限制）；若选择自办可继续调用只读工具并直接回复（不派工、不写 SW 会话）

...
- [ ] 日志落盘：按日期 + session_id 分类存放子 Agent 完整痕迹（写入 `ONEAGENT_HOME/logs/`，格式 jsonl；避免未来 SQLite 存储压力） <!-- id: 6 -->
- [ ] 与技能联动：子 Agent 启动时按步骤任务调用技能召回并注入 Top-K skills（若可用） <!-- id: 7 -->
- [ ] 文件作用域：支持对子 Agent 传入可写 scope（workspace 的子目录集合），并在文件工具层强制校验越界写/改/删 <!-- id: 12 -->

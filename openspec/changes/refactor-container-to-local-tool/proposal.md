...
- **G4. LAN 优先且安全可控**：本地模式默认面向局域网（LAN）使用（便于手机/同网段设备访问），同时默认**不暴露公网**（通过来源 IP 限制/显式开关），并使用一个简单密码作为访问控制。
...
- 默认以“共享密码”作为访问控制（替代 Keycloak/OAuth 登录）。
...
- 认证方案：引入 `AUTH_MODE=password`（默认），用一个简单密码做访问控制；可选 `AUTH_MODE=none` 仅用于开发/离线极简场景（需显式开启）。
...
- **默认认证变化（共享密码）**：不再使用 Keycloak/OAuth 登录；前端需要以密码方式进行访问控制（对齐现有 `Authorization` header 的使用方式）。
...
- 用户在 macOS/Linux（至少其一）上：
  ...
  - 局域网内其它设备可以访问（默认 LAN 监听），但公网访问默认被阻止
...
1. **共享密码的配置形态**：密码通过 `ONEAGENT_PASSWORD` 环境变量、配置文件、还是首次启动交互式设置？是否需要支持运行中修改/轮换？
2. **LAN 与公网的判定与开关**：默认阻止公网访问的策略采用“来源 IP 私网白名单”（推荐）还是“仅监听特定网卡 IP”？是否需要 `--allow-public` 一键显式放开？

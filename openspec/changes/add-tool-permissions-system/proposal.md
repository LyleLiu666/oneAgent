...
## Why
当前 tool 权限是“最小可交付”：`ONEAGENT_DISABLE_TOOL_*` + bash 的历史遗留破坏性命令特例。这带来几个问题：
...
- 移除 bash 的历史遗留破坏性命令特例（不再以单一命令 token 作为权限模型）。
...
- Migration: 默认单机体验保持不变；新增能力以向后兼容方式落地；历史遗留破坏性命令特例将被删除并提示替代方案（策略系统）。

...
- [ ] 2.2 Persist policies (settingsdb/config) + hot reload (best-effort)
- [ ] 2.3 Enforce policy in tool mounting (`tool.Mount/Infos`) and tool execution (toolxml + task runner)
- [ ] 2.4 Add structured deny errors + audit logs (trace + server logs)
...
- [ ] 4.1 Replace `ONEAGENT_BASH_ALLOW_RM` with policy-backed command profiles (e.g., readonly/dev/full)
- [ ] 4.2 Prefer allowlist over blacklist for `bash`/`run_command` executables

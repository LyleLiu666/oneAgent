...
- [ ] 定义 `PLAN.md` 的最小格式（任务 id / status / acceptance / scope=glob），并给出示例模板 <!-- id: 1 -->
- [ ] 实现 plan 解析与写回：读取任务列表、更新 checkbox、保持格式稳定 <!-- id: 2 -->
- [ ] 增加 `plan` 工具：`init/get/mark_done`（mark_done 内部触发 observer 校验） <!-- id: 3 -->
- [ ] 实现 observer runner：独立执行、最小上下文、仅基于文件内容/结构校验交付件（不执行命令验收），返回 pass/fail + 原因 <!-- id: 4 -->
- [ ] 与 subagent 集成：
...
- [ ] observer 权限：默认只读（读文件/搜索/列目录），不允许执行命令 <!-- id: 9 -->
- [ ] E2E：

...
func RunBashNative(ctx context.Context, command string, timeout time.Duration, rootDir string, workDir string) (Result, error) {
	...
	// Fail fast with an actionable error if Landlock isn't available.
	if _, err := landlockABIVersion(); err != nil {
		return Result{}, fmt.Errorf("native sandbox unavailable: %w (use sandbox_mode=docker)", err)
	}

	exe, err := os.Executable()
	if err != nil || strings.TrimSpace(exe) == "" {
		return Result{}, fmt.Errorf("native sandbox unavailable: resolve executable: %w", err)
	}
	...
	cmd.Env = mergeEnv(os.Environ(), map[string]string{
		...
		nativeSandboxStartDirEnv: startDir,
		nativeSandboxCommandEnv:  wrappedCmd,
		"HOME":                   root,
		"PWD":                    root,
		"BASH_ROOT_DIR":          root,
		"TMPDIR":                 tmpDir,

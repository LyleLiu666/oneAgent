...
func TestSearch_UsesRgWhenAvailable(t *testing.T) {
	...
	res, err := Search(context.Background(), cat, "review", Options{MaxResults: 8, Timeout: 10 * time.Second}, lookPath)
	if err != nil {
...
func TestSearch_TokenizesMultiWordQuery(t *testing.T) {
	...
	res, err := Search(context.Background(), cat, "apple notes memo", Options{MaxResults: 8, Timeout: 10 * time.Second}, lookPath)
	if err != nil {
		t.Fatalf("search: %v", err)
	}
...
func TestSearch_FallsBackToGrepWhenRgMissing(t *testing.T) {
	...
	res, err := Search(context.Background(), cat, "hello", Options{MaxResults: 8, Timeout: 10 * time.Second}, lookPath)
	if err != nil {
		t.Fatalf("search: %v", err)
	}
	if res.Backend != "grep" {
